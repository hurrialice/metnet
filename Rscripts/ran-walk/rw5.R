...
write_rds(rw_mat, 'rw_mat_check.rds')
...
trace_nodes <- function(m, top){
    outm <- apply(m,2, function(v){
        names(sort(v, decreasing = T)[1:top])
    })
...
print('made rw_trace')
write_rds(rw_trace, 'rw_trace_check.rds')
rm(rw_mat, pc0, M)
### 3. eval_raw object ######
...
make_eval_raw <- function(g, md, ed, rwt){
    ...
    deg <- value(degf)
    print('get deg'); Sys.time()
    bet <- value(betf)
    print('get bet'); Sys.time()
    ev <- value(evf)
    print('get ev'); Sys.time()
    pr <- value(prf)
    print('get pr'); Sys.time()
    cl <- value(clf)
    print('get cl'); Sys.time()
    pw <- value(pwf)
